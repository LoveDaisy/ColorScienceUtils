...
%                       See colorutil.rgb_compression_validator
% PARAMETER
...
p.addOptional('method', 'Greying', @colorutil.rgb_compression_validator);
...
a0 = solve_equation_binary(fun, zeros(num, 1), zeros(num, 1), ones(num, 1));

% Binary search for a1
fun = @(a, idx) max(colorspace.ictcp2xyz(a(idx) .* ictcp(idx, :) + ...
    (1 - a(idx)) .* ictcp_E(idx, :)) * m / scale, [], 2);
a1 = solve_equation_binary(fun, ones(num, 1), zeros(num, 1), ones(num, 1));

a = min(a0, a1);

ictcp = a .* ictcp + (1 - a) .* ictcp_E;
ictcp(ictcp(:, 1) >= colorspace.pq_inverse_eotf(scale), 2:3) = 0;
