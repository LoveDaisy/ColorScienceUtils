...
elseif strcmpi(ucs, 'cam02jab')
    data = colorspace.rgb2cam02jab(rgb, src_space);
    data = data(:, [2, 3, 1]);
    color_func = @abj_color_func;
else
    if ischar(ucs)
...
function color = abj_color_func(abj)
color = ones(size(abj)) * 0.7;
end
